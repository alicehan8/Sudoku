 ...
 *
 * your name: Alice Han and Janae Perez
...
public class Sudoku {    
    ...
    //will return whether col (first) has value (second)
    ...
    //returns true if it is safe to place number in a specific square 
    private boolean isSafe(int row, int col, int n){

        return(!this.rowHasVal[row][n] && !this.colHasVal[col][n] && !this.subgridHasVal[row/3][col/3][n]);
    }
